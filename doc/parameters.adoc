...
== Debugging a Dynamic Parameter

You can get value of a parameter with a following function:
...
Another way could be to use specific function, that runs every time a
parameter changes. Following example prints new values of the parameter.
...
http://deepsymmetry.org/afterglow/doc/afterglow.show.html#var-add-variable-set-fn.21
